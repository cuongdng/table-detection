...
def table_detection(img_path, xml_path):
    ...
    for bbox in sample_annotations:
        cv2.rectangle(img_annotated, bbox, (0, 0, 255), 1)
...
# {table_detection('./forms/' + i, './forms/32.xml') for i in os.listdir('./forms/') if i.endswith('.png') or i.endswith('.PNG') or i.endswith('.jpg') or i.endswith('.JPG') or i.endswith('.xml') or i.endswith('.XML')}
